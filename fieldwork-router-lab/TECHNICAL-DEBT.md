...
3. **Strengthen boundary types.** Client API responses and several AG-UI/provider events use `any`. Define shared response DTOs and a discriminated union for custom events; validate incoming request shapes at route boundaries. SDK-specific conversion should remain inside each adapter.
