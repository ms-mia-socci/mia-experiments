<script lang="ts">
  ...
  untrack(() => {
    if (attachments) attachmentsContext.replace(attachments);
  });
  ...
  $effect(() => {
    ...
    // Subscribe once; observing the bound prop here would restore stale files
    // whenever the parent clears its draft after RUN_STARTED.
    untrack(() => syncAttachments(attachmentsContext.attachments));
  ...
  $effect(() => {
    ...
    untrack(() => {
      if (next !== undefined && attachmentsContext.attachments !== next) {
        attachmentsContext.replace(next);
      }
    });
  });
    ...
    (formRef) => {
      ...
      let onDrop = (e: DragEvent) => {
        ...
        if (e.dataTransfer?.files && e.dataTransfer.files.length > 0) {
          if (!disabled) attachmentsContext.add(e.dataTransfer.files);
        }
      };
  ...
  let handleSubmit = async (event: SubmitEvent) => {
    ...
    let submittedAttachments = attachmentsContext.attachments.map(
      (attachment) => ({
        ...attachment,
      }),
    );
    if (disabled) return;
    let files = serializeFiles
      ? await createFiles(attachmentsContext.attachments)
      : undefined;
